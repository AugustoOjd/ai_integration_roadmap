...
class ConversationResponse(BaseModel):
    ...
    # explícito que la respuesta es una PROYECCIÓN del modelo: input_tokens_used
...
class MessageRequest(BaseModel):
    ...
    # historial, se persiste y se REENVÍA en cada vuelta del loop y en cada turno
    # futuro. Un solo request encarece la conversación entera para siempre.
...
class PendingApprovalResponse(BaseModel):
    ...
    Lleva el tool_input completo a propósito: quien aprueba tiene que poder ver
...
class ApprovalDecision(BaseModel):
    ...
    vuelta. Los argumentos que se van a ejecutar son los que quedaron persistidos
    en el 202 — si el cliente pudiera mandarlos de nuevo, podría mandar OTROS, y
    "aprobar" pasaría a ser "ejecutar lo que yo diga".
    """
...
class ExecutionStepResponse(BaseModel):
    ...
    is_error: bool
    latency_ms: int | None
    # En NULL salvo en el primer paso de cada vuelta: los tokens son de la llamada
    # al modelo, no de cada tool.
...
class LogResponse(BaseModel):
    ...
    `next_before_id` es el cursor de la página siguiente, y es None cuando no hay
    más. Se devuelve explícito en vez de hacer que el cliente lo deduzca del
    último elemento: si mañana cambia el criterio de paginación, el cliente no se
    entera.
    """
...
class TurnResponse(BaseModel):
    ...
    `answer` es lo único que le interesa al usuario final. Todo el resto está para
    vos: sin iterations, tools_used y usage, un agente en producción es una caja
    negra que no podés ni debuggear ni costear.
    """
