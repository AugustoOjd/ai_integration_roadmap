...
hace falta para que una sesión pueda quedar pausada esperando una aprobación y
...
def crear_sesion(db: Db, user_id: CurrentUser) -> SessionResponse:
    ...
    default, las sesiones abiertas siguen con las reglas con las que empezaron.
    """
    chat = ChatSession(
        user_id=user_id,
...
def ver_sesion(session_id: str, db: Db, user_id: CurrentUser) -> SessionResponse:
    """Si no existe —o no es tuya—, get_session levanta y errors.py hace el 404."""
    return _a_response(get_session(db, session_id, user_id=user_id))
...
def ver_log(
    ...
    """
    # El chequeo de dueño primero: si la sesión no es tuya, ni siquiera llegamos a
    # consultar la traza.
    get_session(db, session_id, user_id=user_id)

    pasos = load_steps(db, session_id, limit=limit, before_id=before_id)
    ...
    return LogResponse(
        ...
        stats=SessionStats(**session_stats(db, session_id)),
    )
...
def mandar_mensaje(
    ...
    """
    chat = get_session(db, session_id, user_id=user_id)

    # Una sesión pausada no acepta mensajes nuevos. Si los aceptara, el turno nuevo
    ...
    # Una sesión agotada se corta acá, sin llegar a count_tokens ni a nada: es el
